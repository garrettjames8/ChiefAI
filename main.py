...
try:
    if OPENAI_API_KEY:
        # Set environment variable for OpenAI to use
        os.environ["OPENAI_API_KEY"] = OPENAI_API_KEY
        client = openai.OpenAI()  # Will automatically use the env var
        logger.info("OpenAI client initialized successfully")
    else:
        logger.warning("OpenAI API key not configured. AI executives will not function.")
except Exception as e:
    logger.error(f"Failed to initialize OpenAI client: {e}")
    logger.error(f"OpenAI library version info available: {hasattr(openai, '__version__')}")
    client = None
...
async def get_executive_response(executive_id: str, message: str, context: str = "") -> str:
    ...
    try:
        if not client:
            return "OpenAI API key not configured. Please add your API key to use AI executives."
